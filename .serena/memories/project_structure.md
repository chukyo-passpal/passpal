...
- `settings/`, `login/`, `setup/`, `debug/`, `force-update.tsx`, `license.tsx`, `maintenance.tsx` など個別フローをファイル単位で配置。
- 画面ファイルは UI ロジックの薄いコンテナとして保ち、hook + component を呼ぶだけにする。

## `src/presentation/`
- `components/` に pure UI。`*.stories.tsx` で Storybook を同期。
- `hooks/` に画面ロジック (`useAssignment`, `useTimetable`, `useClass`, `useMail`, `useNews`, `useAuth`, `useSetting`, `useToast`, `useAppInit`)。
